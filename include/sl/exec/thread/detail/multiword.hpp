...
#define SL_EXEC_DESCRIPTOR_POOL_TEMPLATE_INSTANTIATE(T)                   \
    using T##_state_traits = ::sl::exec::detail::mw::state_traits<T>;     \
    using T##_pointer_traits = ::sl::exec::detail::mw::pointer_traits<T>; \
...
//
// ---
// algorithm was changed to read mutables only, and instead of taking and returning dv - nullopt is employed
// atomics ordering:
// sequence load is acquire - could be relaxed, but read_mutables may be used in "message-passing" context
template <Descriptor DT>
[[nodiscard]] meta::maybe<state_type> read_mutables(pointer_type pointer) {
    using state_impl = state_traits<DT>;
    using pointer_impl = pointer_traits<DT>;

...
//   .default_([](meta::unit) -> int { fmt::println("none were ready"); return -1; } <-- TODO
//
// TODO: try_select should take all locks in consistent order before fetching, so emit would look like:
// select_default::emit() && {
//   auto mutexes = meta::for_each([](auto& connection) { return connection.get_mutex(); });
//   std::lock_guard locks{ mutexes };
//   meta::for_each([](auto&& connection) { std::move(connection).emit(); }, std::move(connections_));
// }
// struct select_default_signal {
...
concept SelectFunctorFor = std::invocable<typename SomeSignalT::value_type>;
...
template <typename ValueT, template <typename> typename Atomic>
...
template <typename ValueT, template <typename> typename Atomic, typename... SelectCaseTs>
struct select_connection : cancel_mixin {
    using value_type = ValueT;
    using error_type = meta::undefined;

    template <typename SelectCase>
    struct case_slot_type : select_slot<typename SelectCase::signal_type::value_type, Atomic> {
...
private:
    ...
    [[nodiscard]] bool try_check_done() {
        return cas2_1({ .a = done_, .o = 0ul, .n = 1ul, .mo = std::memory_order::acq_rel });
    }
...
struct select_connection_box {
    using connection_type = select_connection<ValueT, Atomic, SelectCaseTs...>;
...
public:
    constexpr select(SelectCaseTs&&... cases) : cases_{ std::move(cases)... } {}
    ...
    constexpr auto case_(NextSignalT&& signal, NextF&& functor) && {
        return std::apply(
            [&](SelectCaseTs&&... cases) {
                return select{ std::move(cases)..., NextCaseT{ std::move(signal), std::move(functor) } };
            },
            std::move(cases_)
        );
    }

    // template <SelectFunctorFor<select_default_signal> NextF>
    // constexpr auto default_(NextF&& functor) && {
    //     return std::apply(
    //         [&](SelectCaseTs&&... cases) {
    //             return select_default{ std::move(cases)..., NextCaseT{ select_default_signal{}, std::move(functor) }
    //             };
    //         },
    //         std::move(cases_)
    //     );
...
public: // SomeSignal
    Connection auto subscribe(slot<value_type, error_type>& slot) && {
        return select_connection_box<ValueT, Atomic, SelectCaseTs...>{ std::move(cases_), slot };
    }
...
struct select_start {
    ...
    auto case_(SomeSignalT&& signal, F&& functor) {
        ...
        return select<typename case_type::value_type, Atomic, case_type>{

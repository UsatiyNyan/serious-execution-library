...
struct [[nodiscard]] channel_impl {
    struct send_node_type : channel_node {
        ...
        send_node_type(ValueT&& value, select_slot<meta::unit, Atomic>& select_slot)
            : value_{ std::move(value) }, select_{ select_slot }, slot_{ select_slot } {}

        ValueT& get_value() & { return value_; }
        meta::maybe<select_slot<meta::unit, Atomic>&>& get_select() & { return select_; }
        slot<meta::unit, meta::undefined>& get_slot() & { return slot_; }

    private:
        ValueT value_;
        meta::maybe<select_slot<meta::unit, Atomic>&> select_{};
    ...
    struct receive_node_type : channel_node {
        ...
        explicit receive_node_type(select_slot<ValueT, Atomic>& select_slot)
            : select_{ select_slot }, slot_{ select_slot } {}

        meta::maybe<select_slot<ValueT, Atomic>&>& get_select() & { return select_; }
        slot<ValueT, meta::undefined>& get_slot() & { return slot_; }

    private:
        meta::maybe<select_slot<ValueT, Atomic>&> select_{};
...
public:
    void send(send_node_type& send_node) & {
        ...
        channel_node* q_back = q_.peek_back();
...
private:
    [[nodiscard]] static bool
        ...
        if (send_node.get_select().has_value() && receive_node.get_select().has_value()) {
            ...
            const bool success = cas2(
                { .a = send_select.get_done(), .o = 0ul, .n = 1ul, .mo = std::memory_order::acq_rel },
                { .a = receive_select.get_done(), .o = 0ul, .n = 1ul, .mo = std::memory_order::acq_rel }
            );
...
struct [[nodiscard]] channel_send_signal {
    ...
    struct [[nodiscard]] connection_type : cancel_mixin {
        connection_type(ValueT&& value, slot<value_type, error_type>& slot, impl_type& impl)
            : node_{ std::move(value), slot, impl }, impl_{ impl } {
            slot.intrusive_next = this;
        }

        connection_type(ValueT&& value, select_slot<value_type, Atomic>& select_slot, impl_type& impl)
            : node_{ std::move(value), select_slot, impl }, impl_{ impl } {
...
public:
    ...
    Connection auto subscribe(select_slot<value_type, Atomic>& select_slot) && {
...
struct [[nodiscard]] channel_receive_signal {
    ...
    struct [[nodiscard]] connection_type : cancel_mixin {
        connection_type(slot<value_type, error_type>& slot, impl_type& impl) : node_{ slot, impl }, impl_{ impl } {
            slot.intrusive_next = this;
        }

        connection_type(select_slot<value_type, Atomic>& select_slot, impl_type& impl)
            : node_{ select_slot, impl }, impl_{ impl } {
            select_slot.intrusive_next = this;
        }
...
template <typename ValueT, typename Mutex = std::mutex, template <typename> typename Atomic = detail::atomic>

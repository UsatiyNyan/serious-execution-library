...
executor::~executor() noexcept {
    if (ASSUME_VAL(queue_.empty(), "executor destroyed with unfinished tasks", queue_.size())) {
        return;
    }
...
void executor::schedule(generic_task_node* task_node) noexcept {
    if (ASSUME_VAL(task_node != nullptr) && !is_stopped_) {
        queue_.push_back(task_node);
    }
}

std::size_t executor::execute_batch() noexcept {
...
std::size_t executor::execute_at_most(std::size_t n) noexcept {
